...
public class PromptChapa extends Dialog {
	...
	private ArrayList<HashMap<String, String>> chapasList;// = new
	...
	public PromptChapa(Context context,
			ArrayList<HashMap<String, String>> listChapas,
			final NumeroChapaCallback callback) {
		super(context);
		this.chapasList = listChapas;
		this.callback = callback;

		SharedPreferences SP = PreferenceManager
				.getDefaultSharedPreferences(DisplayManager.getInstance()
						.getContext());
		final int promptChapaTiemeout = Integer.parseInt(SP.getString(
				"prefPromptChapaTimeout", "20"));

		InputManager.getInstance().startTimer(promptChapaTiemeout,
				new TimerCallBack() {
					@Override
					...
					public void onTick(long millisUntilFinished) {
						...
						textViewTimer.setText(String.format("%02d", Integer
								.valueOf(String
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		adapter = new LazyAdapterChapas(DisplayManager.getInstance()
				.getContext(), chapasList);
	...
	public boolean onKeyDown(int keyCode, KeyEvent event) {
		try{
			...
			if (key != null && key.length() > 0){
				...
				if (numericOption <= adapter.getCount() && numericOption > 0){
